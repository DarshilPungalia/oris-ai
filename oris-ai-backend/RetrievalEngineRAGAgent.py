...
async def retrieval_dental_info(context: RunContext, query: str) -> dict:
    ...
    try:
        ...
        nodes = await loop.run_in_executor(None, sync_retrieve)
        
        if not nodes:
            print("❌ No relevant information found in knowledge base")
            return {"result": "I don't have specific information about that in my knowledge base. However, I'd recommend scheduling an appointment with our dental team for a personalized consultation."}
        
        context_str = "\n\n".join([node.get_content(metadata_mode=MetadataMode.LLM) for node in nodes])
        print(f"✅ Retrieved {len(nodes)} relevant documents")
        print("📄 Retrieved context preview:", context_str[:200] + "..." if len(context_str) > 200 else context_str)
        return {"result": context_str}
    except Exception as e:
        print(f"❌ Error retrieving dental information: {e}")
